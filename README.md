...
CSS는 **Cascading Style Sheets**의 약자로, 스타일링을 위한 언어입니다. **Cascading**이란 한 엘리먼트에 여러 스타일이 적용될 경우 스타일 간 충돌을 막기 위해 계단식으로 적용하는 규칙을 의미합니다. 하나의 엘리먼트에 여러 개의 스타일이 적용될 수 있고, 그 반대도 가능합니다. 엘리먼트에 스타일이 적용되는 규칙을 **선택자(Selector)**라고 합니다. 선택자는 HTML 엘리먼트에 직접 넣을 수도 있고, 엘리먼트의 조합 혹은 클래스의 형태로 작성할 수 있습니다. 스타일은 **property**와 **value**의 형태로 이루어지며, 이들은 `:`으로 구분되고 각 스타일은 `;`으로 구분합니다.

### 상태 선택자(State Selector)

상태 선택자(`::`)는 해당 엘리먼트에 특수한 상호작용을 설정합니다. 예를 들어, `hover`, `active`, `focus` 등이 있습니다.

### 레이아웃 관련 속성

레이아웃과 관련된 속성은 화면에 엘리먼트가 어떻게 배치될 것인지를 결정합니다. 가장 중요한 속성은 **Display**입니다.

- `none`: 엘리먼트가 존재하지만 보이지 않습니다. 자바스크립트로 제어할 때 주로 사용됩니다.
- `block`: 세로로 정렬되며, `width`와 `height` 값을 가질 수 있습니다. 크기에 상관없이 한 줄을 차지합니다. 대표적으로 `div` 태그가 있습니다.
- `inline`: 가로로 정렬되며, `width`와 `height` 값을 가질 수 없습니다. 컨텐츠의 크기만큼 공간을 차지합니다. 대표적으로 `p` 태그가 있습니다.
- `inline-block`: `inline`의 특성을 가지면서도 `width`와 `height` 등을 제어할 수 있습니다.
- `flex`: 컨테이너 형태로 엘리먼트를 관리합니다.
- `grid`: `flex`가 1차원적인 반면, `grid`는 2차원적으로 엘리먼트를 관리할 수 있습니다.

### visibility 속성

**visibility** 속성은 엘리먼트의 가시성을 정의합니다. `visibility: hidden`과 `display: none`의 차이는 전자는 엘리먼트의 영역이 남아 있고, 후자는 완전히 제거된다는 점입니다.

### position 속성

**position** 속성은 엘리먼트를 어떻게 위치시킬지를 정의합니다.

- `static`: 기본 위치, 원래 순서대로 배치됩니다.
- `relative`: 자신의 원래 위치를 기준으로 상대적으로 배치됩니다.
- `absolute`: 가장 가까운 위치 지정 조상 요소를 기준으로 절대적으로 배치됩니다.
- `fixed`: 브라우저 창을 기준으로 절대적으로 배치됩니다.

### 글꼴 관련 속성

글꼴과 관련된 속성으로는 `font-size`, `font-weight`, `font-style` 등이 있습니다.

### Styled-Components

**Styled-Components**는 CSS 문법을 그대로 사용하면서 결과물을 스타일링된 컴포넌트 형태로 만들어 주는 오픈 소스 라이브러리입니다. 리액트 개발에서 매우 유용하게 사용됩니다.

#### 설치 방법

Styled-Components를 설치하려면 다음 명령어를 사용합니다:
